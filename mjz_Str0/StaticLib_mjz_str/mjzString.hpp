...
class mjz_Str_DATA_storage_cls
 ...
 protected:
  std::function<void(void*)> free_fnctn = ::free;
  std::function<void*(void*, size_t)> realloc_fnctn = ::realloc;
...
class mjz_Str : public if_virtual_then_virtual Stream {  //
 ...
 protected:
  ...
  // void (mjz_Str::*update_event_F_p)( void); //depated             // to call
 ...
 //private:
  void* realloc_pv(void* ptr, size_t new_size);
  void free_pv(void*& ptr);
  void free_pv(void* const& ptr);
 protected:
  virtual void* realloc(void* ptr, size_t new_size) {
  if(!did_drived_mjz_Str_DATA_storage_Obj_ptr_set)  return ::realloc(ptr, new_size);
 ...
 public:
  ...
  if_virtual_then_virtual size_t UN_ORDERED_compare(const char* s,
                                                    size_t s_len) const;
  if_virtual_then_virtual size_t UN_ORDERED_compare(const mjz_Str& s) const;
  ...
  inline const std::function<bool(char)>& get_is_blank_character_function() const {
  ...
  inline const std::function<void*(void*, size_t)>& get_realloc_alloc_function()
      const {
...
*/
  ...
  if_virtual_then_virtual int64_t lastIndexOf(const mjz_Str& str,
                                              size_t fromIndex) const;
  if_virtual_then_virtual const char* substring_give_ptrULL(size_t left,
                                                         size_t right,
  ...
  if_virtual_then_virtual mjz_Str substring(unsigned int beginIndex,
  ...
  if_virtual_then_virtual mjz_Str substring(int beginIndex,
                                            int endIndex) const {
  ...
  mjz_Str& push_back(char cr_) { write(cr_); }
...
inline mjz_Str operator"" _m_str(unsigned long long num) { return mjz_Str(num); }
inline mjz_Str operator"" _m_str(long double num) { return mjz_Str((double)num); }
