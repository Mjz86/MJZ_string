...
// but better
	...
	public:
		...
		[[nodiscard]] static std::shared_ptr<mjz_Str_DATA_storage_cls> create() {
			...
			// private.
			return std::shared_ptr<mjz_Str_DATA_storage_cls>(
				new mjz_Str_DATA_storage_cls());
		}
	...
	protected:
		std::shared_ptr<mjz_Str_DATA_storage_cls>
			...
			main_mjz_Str_DATA_storage_Obj_ptr->getptr();
		...
		size_t len;       // the mjz_Str length (not counting the '\0')
		...
		static constexpr int64_t stack_buffer_size = 33;
		class stack_str_buf {
		...
		public:
			...
			stack_str_buf() : STR_is_in_stack(0) {
				for (int64_t i{}; i <= stack_buffer_size; i++) {
					stack_buffer[i] = 0;
				}
			}
			...
			virtual ~stack_str_buf() { STR_is_in_stack = 0; }
		};
	...
	public:
		int getWriteError() if_override_then_override { return write_error; }
		void clearWriteError() if_override_then_override { setWriteError(0); }

		// default to zero, meaning "a single write may block"
		// should be overridden by subclasses with buffering
		virtual int availableForWrite() if_ard_then_override { return (int)availableForWriteLL(); }
		...
		inline static void operator delete(void* ret_piont, void* where) {
		...
		mjz_Str(const char* cstr, size_t length);
		mjz_Str(const uint8_t* cstr, size_t length)
		...
		mjz_Str() : mjz_Str((const char*)empty_STRING_C_STR) {}
#ifndef Arduino
		...
		if_virtual_then_virtual explicit operator const size_t() const { return len; }
		if_virtual_then_virtual explicit operator size_t() const { return len; }
		if_virtual_then_virtual explicit operator size_t& () { return (size_t&)len; }
		...
		if_virtual_then_virtual bool concat(const uint8_t* cstr,
			size_t length) {
		...
		if_virtual_then_virtual int64_t compareTo(const mjz_Str& s) const;
		...
		if_virtual_then_virtual unsigned long UN_ORDERED_compare(
			const char* s, size_t s_len) const;
		if_virtual_then_virtual unsigned long UN_ORDERED_compare(
			const mjz_Str& s) const;
		...
		static mjz_Str create_mjz_Str_char_array(size_t size_, char filler = 0,
			bool do_fill = 1);
		static mjz_Str create_mjz_Str_2D_char_array(size_t size_col,
			size_t size_row,
			char filler = 0,
			bool do_fill = 1);
		...
		friend bool operator==(const char* a, const mjz_Str& b) { return b == a; }
		friend bool operator<(const mjz_Str& a, const mjz_Str& b) {
		...
		friend bool operator!=(const mjz_Str& a, const char* b) { return !(a == b); }
		friend bool operator!=(const char* a, const mjz_Str& b) { return !(a == b); }
		friend bool operator>(const mjz_Str& a, const mjz_Str& b) { return b < a; }
		friend bool operator>(const mjz_Str& a, const char* b) { return b < a; }
		friend bool operator>(const char* a, const mjz_Str& b) { return b < a; }
		friend bool operator<=(const mjz_Str& a, const mjz_Str& b) {
			return !(b < a);
		}
		...
		if_virtual_then_virtual bool startsWith(const mjz_Str& prefix,
			size_t offset) const;
		...
		if_virtual_then_virtual void getBytes(unsigned char* buf,
			size_t bufsize,
			size_t index = 0) const;
		inline if_virtual_then_virtual void toCharArray(
			char* buf, size_t bufsize, size_t index = 0) const {
		...
		inline if_virtual_then_virtual const char* c_str() { return buffer_ref(); }
		inline if_virtual_then_virtual const char* C_str() const {
			return buffer_ref();
		}

		inline if_virtual_then_virtual char* C_str() { return buffer_ref(); }

		inline if_virtual_then_virtual char* begin_c_str() { return buffer_ref(); }
		...
		public:
		  ...
		  const_iterator &operator=(iterator_template_CC p) {
			m_iterator = p.get_pointer(); // there is a error if i want  to access it
	  ...
	  */
		...
		mjz_Str& erase_from_f_to_l(long first, long last);
		// search
		...
		if_virtual_then_virtual int64_t lastIndexOf(char ch,
			size_t fromIndex) const;
		...
		if_virtual_then_virtual mjz_Str substring(size_t beginIndex) const {
		...
		if_virtual_then_virtual mjz_Str substring(int64_t beginIndex, int64_t endIndex = -1) const {
		...
		if_virtual_then_virtual mjz_Str substring(unsigned int beginIndex) const {
			return substring((size_t)beginIndex);
		};
		if_virtual_then_virtual mjz_Str substring(unsigned int beginIndex, unsigned int endIndex) const {
			return substring((size_t)beginIndex, (size_t)endIndex);
		}
		if_virtual_then_virtual mjz_Str substring_beg_n(unsigned int beginIndex, unsigned int number) const {
		...
		if_virtual_then_virtual mjz_Str substring_beg_n(int beginIndex, int number) const {
		...
		if_virtual_then_virtual mjz_Str substring(TYPE_ beginIndex, TYPE_2 endIndex) const {
			return substring((int64_t)beginIndex, (int64_t)endIndex);
		}
		template<typename TYPE_, typename TYPE_2>
		if_virtual_then_virtual mjz_Str substring_beg_n(TYPE_ beginIndex, TYPE_2 number) const {
			return substring_beg_n((int64_t)beginIndex, (int64_t)number);
		}
		...
		void swap(mjz_Str& rhs) {
			mjz_Str& lhs = *this;
			mjz_Str lhs_buffer(std::move(lhs));//be carefull
			new (&lhs) mjz_Str;//be carefull
			lhs = std::move(rhs);
			rhs = std::move(lhs_buffer);
		}

		const char& at(int64_t i) const {
			return operator[](i);
		}
		char& at(int64_t i) {
			return operator[](i);
		...
		mjz_Str& insert(size_t pos, const mjz_Str& other, size_t subpos, size_t sublen = -1);
	...
	protected:
		...
		if_virtual_then_virtual bool changeBuffer(size_t maxStrLen);
