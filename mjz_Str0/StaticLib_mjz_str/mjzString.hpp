...
class mjz_Str : public if_virtual_then_virtual Stream {  //
 ...
 public:
  ...
  if_virtual_then_virtual explicit operator std::string &&() const {
    return std::move(std::string((const char *)buffer_ref(),length()));
  }
...
class malloc_wrpr {
 ...
 public:
  malloc_wrpr(size_t size_of_ptr = 0) { m_data_ptr= malloc(size_of_ptr); };
  malloc_wrpr(size_t size_of_ptr, int VAl_) {
    m_data_ptr = malloc(size_of_ptr);
    memset(VAl_);
  };

  ~malloc_wrpr() { free(); }
  malloc_wrpr(malloc_wrpr &) = delete;
  malloc_wrpr(malloc_wrpr &&) = delete;
  malloc_wrpr(const malloc_wrpr &) = delete;
  malloc_wrpr &operator=(malloc_wrpr &) = delete;
  malloc_wrpr &operator=(malloc_wrpr &&) = delete;
  malloc_wrpr &operator=(const malloc_wrpr &) = delete;
