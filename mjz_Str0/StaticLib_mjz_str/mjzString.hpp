...
class mjz_String_memory_class : public mjz_str_DB_t<Char_t_> {
 public:
  ...
  inline _NODISCARD void *A_mjz_realloc(void *pr_real_ptr,
                                        size_t preveious_size,
                                        size_t new_size) noexcept {
  ...
  inline static constexpr size_t real_size_to_capacity(size_t real_size) {
    return ((real_size - sizeof(sheared_int_t)) / sizeof(Char_t_)) -
           B_t::nullen;
  }
  ...
  inline static bool real_ptr_is_sheared(bool is_dynamic,
                                         bool DF0_can_be_sheared,
  ...
  inline static states real_ptr_is_state(bool is_dynamic,
                                         bool DF0_can_be_sheared,
                                         bool DF1_has_atomic, void *real_ptr) {
  ...
  inline static constexpr _NODISCARD size_t to_optimal_cap(size_t new_cap,
    ...
    return max(new_cap,
               max(64, max(pr_cap + pr_cap - pr_len, pr_cap + pr_len)));
  }
  ...
  inline _NODISCARD succsess_t realloc_memory(size_t new_cap, bool force_shrink,
    ...
    bool SF0_can_share =
        B().get_static_flag<B_t::S_flag0_will_allocate_a_sharable_string>();
    bool can_throw =
        can_throw_ && B().get_static_flag<B_t::S_flag1_can_throw_if_error>();
    bool alloc_more =
        force_shrink
            ? 0
    ...
    if (past.was_static) {
      ...
      void *real_ptr =
          A_mjz_realloc(nullptr, 0, capacity_to_real_size(new_cap));
      if (!real_ptr) {
    ...
    void *real_ptr =
        A_mjz_realloc(others_real_ptr, capacity_to_real_size(past.m_capacity),
  ...
  share_my_dynamic_string_with(mjz_String_memory_class &other_str) {
    ...
    other_str.B().set_all_to_dynamic(B().get_dynamic_string(),
                                     B().get_dynamic_length(),
                                     B().get_dynamic_capacity(), true, true);
  ...
  inline _NODISCARD succsess_t add_length(size_t add_len,
                                          Char_t_ fill_val = '\0',
                                          bool fill = true) {
