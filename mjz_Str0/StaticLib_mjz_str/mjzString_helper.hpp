...
#define MJZ_frm_stack_ovf_BIT_CHECK(a, b) (!!((a) & (1ULL << (b))))  // '!!' to make sure this returns 0 or 1
...
#include <string.h>
template <typename... T>
int sprintf_alt_(char* const buffer_for_print,size_t size_of_msg, const char* const formaing__,
                 T... arggs) {
#ifndef Arduino
...
#define ard_bitWrite(value, bit, bitvalue) ((bitvalue) ? bitSet(value, bit) : bitClear(value, bit))

#define ard_min(a, b) ((a) < (b) ? (a) : (b))
#define ard_max(a, b) ((a) > (b) ? (a) : (b))
#define ard_abs(x) ((x) > 0 ? (x) : -(x))
#define ard_constrain(amt, low, high) ((amt) < (low) ? (low) : ((amt) > (high) ? (high) : (amt)))
