...
#define PGM_P const char *
	...
	long long mjz_Str::to_LL(int radix, bool* had_error,
		...
		if (buffer) {
			return C_STR_to_LL(c_str(), (uint8_t)min(length(),(uint32_t)255), radix, had_error, error_level);
		}
	...
	*/
	...
	const mjz_Str& helper__op_shift_input_(const mjz_Str& rhs, const mjz_Str& CIN, mjz_Str& get_shift_op_s) {
		...
		register  uint32_t CURunt_index_{};
		...
		auto continue_event_is_reinterpreted_and_is_int = [&](register  uint16_t& i) -> bool {
		...
		auto continue_event_is_reinterpreted =
			[&](register  uint16_t& i, uint8_t& is_reinterpreted_do_not_forbid) -> bool {
			int8_t vlal_bf = rhs.char_to_int_for_string(bfr[i]);
