...
int64_t mjz_Str::compareTo(const mjz_Str& s) const {
  if (!buffer || !s.buffer) {
    if (s.buffer && s.len > 0) {
      //// (this->*update_event_F_p)(); //depated
      return static_cast<int64_t>(0) - *(unsigned char*)s.buffer;
    }
...
int64_t mjz_Str::compareTo(const char* cstr) const {
  if (!buffer || !cstr) {
    if (cstr && *cstr) {
      // // (this->*update_event_F_p)(); //depated
...
bool mjz_Str::startsWith(const mjz_Str& s2, size_t offset) const {
...
char mjz_Str::operator[](size_t index) const {
  if (index >= len || !buffer) {
    // // (this->*update_event_F_p)(); //depated
    return 0;
  }
...
void mjz_Str::getBytes(unsigned char* buf, size_t bufsize, size_t index) const {
  if (!bufsize || !buf) {
    // // (this->*update_event_F_p)(); //depated
    return;
  }
...
int64_t mjz_Str::indexOf(char c) const { return indexOf(c, 0); }

int64_t mjz_Str::indexOf(char ch, size_t fromIndex) const {
  if (fromIndex >= len) {
    return -1;
  }
...
int64_t mjz_Str::lastIndexOf(char ch, size_t fromIndex) const {
  if (fromIndex >= len) {
    return -1;
  }
...
int64_t mjz_Str::lastIndexOf(const mjz_Str& s2) const {
...
bool mjz_Str::is_blank() const {
  for (size_t index_i{}; index_i < len; index_i++) {
