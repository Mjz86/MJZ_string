...
namespace std::chrono {
  namespace mjz_ard {
    unsigned long long millisR() {
      ...
      return ms.count(); } } // namespace mjz_ard
}  // namespace std::chrono
#endif
#ifndef Arduino
  ...
  long long C_STR_to_LL( const char * buffer, uint8_t buffer_len, int radix,
    ...
    while ( WHILE_Index_for_buffer < buffer_len ) {
      ...
      if ( is_neg_bool_buf ) {
        ...
        if ( buffer_for_number[WHILE_Index_for_int_buf] == 127 ) {
          ...
          if ( buffer_for_number[WHILE_Index_for_int_buf] >= radix ) {
            *had_error = 1;
    ...
    for ( uint16_t i{0 }; i < 2047; i++ ) {
      ...
      if ( is_reinterpreted == is_reinterpreted_and_is_int ) {
        ...
        if ( is_reinterpreted == 1 ) {
          ...
          if ( rhs.drived_mjz_Str_DATA_storage_Obj_ptr->reinterpret_char_char ==
               bfr[i] ) {
            bfr[i] = '\0';
            i--;
            is_reinterpreted = 1;
