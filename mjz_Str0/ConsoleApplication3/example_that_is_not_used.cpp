...
void test_mstr_vs_sstr(
  ...
  auto* view_storage = reinterpret_cast<mjz_str_view*>(a_storage);
...
activation_function( std::vector<float> sum) {
...
inline float calculate_expected_input(float input_at_i,
                                      float delta_weight,float err) {
  return delta_weight/err +
      input_at_i;
}
inline float calculate_sum(uint32_t input_count,
...
inline std::vector < float> calculate_sum(uint32_t input_count,
                           const std::vector<float>& inputs,
...
inline void  change_weights(uint32_t input_count,
                          std::vector<float>& expected_inputs,
                                         const std::vector<float>& inputs,
                            std::vector<float>& weights,
                            float sum_of_in_times_w,
                             float pervious_output,
  ...
  for (uint32_t i{}; i < input_count;) {
    float delta_weight =
      ...
      expected_inputs[i] = calculate_expected_input(
          inputs[i], delta_weight, error_derivitive_multiplier);
...
inline std::vector<float> change_weights_in_layer(
        ...
        const std::vector<float>& inputs,
                                                  ...
                                                  std::vector < float> sum_of_in_times_w,
                                                  std::vector<float> pervious_output,
...
std::vector<float> calculate_one_layer(std::vector<std::vector<float>>& weights, std::vector<float> inputs,
    ...
    error_cost += pow(outputs[j] - ex_output, 2);
      ...
      std::vector<float> expected_inputs =
          change_weights_in_layer(inputs.size(), weights.size(), inputs,
                              weights,
                              sum, outputs,
                              expected_outputs);
      return expected_inputs;
    }
std::vector<float> forward_propgate_one_layer(std::vector<std::vector<float>>& weights,
...
std::vector<float> pridict(std::vector < std::vector<std::vector<float>>> &
                               weights,
                           std::vector<float> inputs) {
...
double back_propagate(
        std::vector < std::vector<std::vector<float>>> & weights,
        std::vector<float> inputs,
        std::vector<float> expected_outputs_for_net ) {
      double error_cost{};
      uint32_t layer_count = weights.size();
      std::vector<std::vector<float>> sums(layer_count);
      std::vector<std::vector<float>> outputs(layer_count);
      outputs[0] = forward_propgate_one_layer(weights[0], inputs, sums[0]);
      for (uint32_t i{1}; i < layer_count; i++) {
    outputs[i]= forward_propgate_one_layer(weights[i], outputs[i - 1],
                                              sums[i]);
      }
      uint32_t last_layer = layer_count - 1;
      std::vector<std::vector<float>> expected_outputs(layer_count-1);
      expected_outputs[last_layer - 1] =
          calculate_one_layer(weights[last_layer], outputs[last_layer-1],
                              expected_outputs_for_net, sums[last_layer],
                              outputs[last_layer], error_cost);

      for (uint32_t i{last_layer-1}; 0 < i; i--) {
    expected_outputs[i - 1] = calculate_one_layer(weights[i], outputs[i - 1], expected_outputs[i],
        sums[i], outputs[i], error_cost);
          }
      calculate_one_layer(weights[0], inputs, expected_outputs[0],sums[0], outputs[0], error_cost);
          return error_cost;
    }
  ...
  while (1) {
    ...
    if (abs(error_cost - pervious_error) < 0.00000001) break;
